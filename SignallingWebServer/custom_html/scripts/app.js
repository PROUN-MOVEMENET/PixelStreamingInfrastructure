...
let matchViewportResolution = true; // PROUN CHANGE //
...
function onInputControlOwnership(data) {
    ...
    const ownership = view[1] !== 0;
    console.log(
        `Received input controller message - will your input control the stream: ${ownership}`,
    );
...
function parseURLParams() {
    ...
    inputOptions.controlScheme = ControlSchemeType.HoveringMouse; // PROUN CHANGE //
...
function setupHtmlEvents() {
    ...
    setupToggleWithUrlParams("offer-receive-tgl", "offerToReceive"); */ // PROUN CHANGE //
...
function showFreezeFrame() {
    const base64 = btoa(
        freezeFrame.jpeg.reduce((data, byte) => data + String.fromCharCode(byte), ""),
    );
...
function setupStats() {
    ...
    webRtcPlayerObj.onAggregatedStats = (aggregatedStats) => {
        ...
        statsText += `<div>Duration: ${timeFormat.format(runTimeHours)}:${timeFormat.format(
            runTimeMinutes,
        )}:${timeFormat.format(runTimeSeconds)}</div>`;
...
function getKeyCode(e) {
    ...
    if (e.keyCode === SpecialKeyCodes.Alt && e.code === "AltRight") return SpecialKeyCodes.RightAlt;
    return e.keyCode;
}
...
function connect() {
    ...
    ws.onclose = function (event) {
        ...
        if (ws.attemptStreamReconnection === true) {
            ...
            const reclickToStart = setTimeout(() => {
                start(true);
            }, 4000);
        }
